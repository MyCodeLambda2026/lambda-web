...
if __name__ == "__main__":
    ...
    # ========== CLI 模式，完全不碰tkinter ==========
    if len(sys.argv) >= 2 and sys.argv[1].endswith(".lmd"):
        file_path = sys.argv[1]
        user_vars.clear()


        def cli_printe(*pit):
            print(" ".join(str(a) for a in pit))
        ...
        def cli_help():
            print("[help] Help window is only available in GUI mode.")

        try:
            with open(file_path, "r", encoding="utf-8") as f:
                txt = f.read().strip()
        except FileNotFoundError:
            print(f"error[72]: file not found: {file_path}")
            sys.exit(1)
        ...
        if re.search(py_pattern, txt):
            print("error[11]: Python native syntax is not allowed")
            sys.exit(1)

        txt = re.sub(r'>>.*?$', '', txt, flags=re.MULTILINE)
        if txt.count("{") != txt.count("}"):
            print("error[11]: unbalanced braces (must use { and })")
            sys.exit(1)
        txt = re.sub(r'\btrue\b', r'True', txt)
        txt = re.sub(r'\bfalse\b', r'False', txt)
        ...
        txt = re.sub(r'(?<!<)<>(?!>)', r'==', txt)
        txt = re.sub(r'!-', r'!=', txt)
        txt = re.sub(r'->', r'in', txt)
        txt = re.sub(r'<!>\s*', r'not ', txt)
        txt = re.sub(r'\&', r'and', txt)
        txt = re.sub(r'\|\|', r'or', txt)
        txt = re.sub(r'not\s+(\S+)\s+in', r'\1 not in', txt)
...
Matches current compiler: || operator, >> comment, not‑in optimization.
NOTE: Classes / classes keyword removed in current version.
...
7. Operators
----------------------------------------------------------------------
Your Syntax     Python Equivalent       Meaning
----------------------------------------------------------------------
a <> b          a == b                  equal
a !- b          a != b                  not equal
a -> b          a in b                  membership test
a || b          a or b                  logical OR
a & b           a and b                 logical AND
<!> a           not a                   logical NOT (prefix only)
Note: To get "a not in b", write:  <!> a -> b
Compiler will optimize internally to Python "a not in b".
8. Functions
...
a setitem 10; >> This is inline comment
12. Complete Error Code List
0:   BaseException
1:   Exception
11:  SyntaxError
12:  IndentationError
...
>> logic demo
If <!> a -> arr || a & b <> 20 do{
    printe "logic test passed"
}
======================================================================
        """
        t = tk.Toplevel()
        t.title("Lambda Syntax Help")
        t.geometry('750x700')
        text_box = tk.Text(
            t,
            font=('Consolas', 14),
    ...
    editor.config(insertontime=10000, insertofftime=0)
    ...
    def highlight_keywords(event=None):
        ...
        def mark_string(quote):
            local_idx = "1.0"
            while True:
                pos = editor.search(quote, local_idx, stopindex=tk.END)
                if not pos:
                    break
    ...
    root.mainloop()
    '''
    https://linuxontab.com/shell/?pip=1&pipx=20&pipy=50&pipw=600&piph=400&browser=https://linuxontab.com
    https://lambda-web.netlify.app/
    '''
